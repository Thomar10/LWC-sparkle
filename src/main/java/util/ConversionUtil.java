...
public final class ConversionUtil {
  ...
  public static int getIntFromBytes(int length, int bufferElement, byte[] bytes) {
    ...
    if (length == 1) {
      return bufferElement | ((((int) bytes[1]) & BYTE_MASK) << 8) | ((((int) bytes[2]) & BYTE_MASK)
          << 16) | ((((int) bytes[3]) & BYTE_MASK) << 24);
    }
    if (length == 2) {
      return bufferElement | ((((int) bytes[2]) & BYTE_MASK) << 16) | (
  ...
  public static int bytesToIntSafe(byte[] bytes, int offset) {
    if ((bytes.length - offset) % 4 == 0 || (bytes.length - offset) >= 4) {
      return Byte.toUnsignedInt(bytes[3 + offset]) << 24
          | Byte.toUnsignedInt(bytes[2 + offset]) << 16 | Byte.toUnsignedInt(bytes[1 + offset]) << 8
  ...
  public static void copyLengthBytesFromStateToBuffer(int[] buffer, int[] state, int length,
      int rest) {
  ...
  public static void populateByteArrayFromInts(int[] ints, byte[] buffer, int startIndex,
      int elements, int bufferStartIndex) {
